...
生成前先询问：赛制(CUMCM电子版/其他)、是否目录(~2页)、摘要体例(分问「针对问题」仅当确有问题一/二/三；否则主题叙述体)。

生成前先询问使用者意图（详见 `../mathmodel-cumcm-style/SKILL.md`）：
1. 赛制：CUMCM 电子版 / 其他竞赛或专题论文
2. 是否目录（有则约 2 页；CUMCM 电子版默认无目录）
3. 摘要体例：仅当确有「问题一/二/三」才用「针对问题…」；否则用主题叙述体铺满一页

硬性要求摘要：
- 按用户意图选择模板；**勿在无分问稿件中虚构「针对问题一/二/三」**
- 摘要独占并尽量铺满一页；关键词 3–5；摘要禁公式图表
- 标题与正文之间加大间距，避免拥挤
- 页边距 ≥2.5cm；页脚居中阿拉伯数字（通常自摘要页=1）
- 字体默认：题目三号黑 / 一级四号黑居中 / 二三级小四黑左齐 / 正文小四宋 / 图表题五号宋
- **同时交付 PDF + Word + LaTeX**
- 主结果用三线表；正文（不含附录）至少约 20 页；国赛电子版正文 ≤30
- 观感学习勿堆参考卷大段代码
