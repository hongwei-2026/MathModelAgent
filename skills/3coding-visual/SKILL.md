...
**期刊感复杂统计图（可选升级）**：若需要 SHAP 蜂群柱状、配对云雨、CV-ROC、泰勒图、相关矩阵组合、预测-真实边缘分布、环形热图、和弦图等，读取并执行 `../mathmodel-figure-templates/SKILL.md`：先跑模板拿版式，再把脚本里的模拟数据换成**本任务真实结果**，最终 PDF 仍写入 `figures/`，并在 `RESULTS_REPORT.md` 记录数据来源。版式与插图纪律对齐 `../mathmodel-cumcm-style/SKILL.md` §4.1–4.2。
